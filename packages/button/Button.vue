...
export default {
  ...
  created() {},
  mounted() {
    const { color, size, plain } = this;

    let bgColor = "";
    let fontColor = "";
    if (color) bgColor = color;
    if (color) fontColor = "#fff";
    if (plain) fontColor = color;
    if (color && plain) {
      bgColor = "#fff";
      fontColor = color;
    }
    if ((color || color.indexOf("gradient") !== -1) && !plain) {
      fontColor = "#fff";
    }
    this.styleObj = {
      background: bgColor,
  ...
  methods: {
    handleBtnClick(e) {
      const button = window.getComputedStyle
        ? window.getComputedStyle(e.currentTarget, null)
        : e.currentTarget.currentStyle;
      const borderColor = button.borderColor;
      document.styleSheets[0].addRule(
        ".is-extra::after",
        `border: ${this.effectWidth} solid ${borderColor};`
      );
      // 控制动画
